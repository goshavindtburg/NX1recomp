...
class XSocket : public XObject {
 ...
 private:
  ...
  bool upnp_mapping_active_ = false;
  uint16_t upnp_native_port_ = 0;

  bool broadcast_socket_ = false;
  bool connected_ = false;
  N_XSOCKADDR_IN connected_to_ = {};
  uint32_t connected_send_log_count_ = 0;
  uint32_t sendto_log_count_ = 0;
...
// Stops the System Link broadcast bridge from spawning further background
// session refreshes and waits (bounded) for any in-flight refresh thread to
// finish, so no detached thread touches runtime globals during shutdown. Safe to
// call multiple times. Must be called before KernelState is torn down.
