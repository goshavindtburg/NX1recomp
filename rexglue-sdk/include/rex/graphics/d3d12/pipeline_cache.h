...
class PipelineCache {
 ...
 private:
  ...
  REXPACKEDSTRUCT(PipelineDescription, {
    ...
    uint32_t nx1_native_shader_cache : 1;             // 12
    ...
    static constexpr uint32_t kVersion = 0x20260606;
  });
  ...
  struct PipelineRuntimeDescription {
    static constexpr uint32_t kNx1NativeMaxInputElements = 32;
    ...
    PipelineDescription description;
    D3D12_SHADER_BYTECODE nx1_native_vertex_shader;
    D3D12_SHADER_BYTECODE nx1_native_pixel_shader;
    D3D12_INPUT_ELEMENT_DESC nx1_native_input_elements[kNx1NativeMaxInputElements];
    uint32_t nx1_native_input_element_count;
    uint32_t nx1_native_vertex_buffer_mask;
  };
  ...
  bool TryConfigureNx1NativePipeline(
      D3D12Shader::D3D12Translation* vertex_shader, D3D12Shader::D3D12Translation* pixel_shader,
      const PrimitiveProcessor::ProcessingResult& primitive_processing_result,
      uint32_t normalized_color_mask, uint32_t bound_depth_and_color_render_target_bits,
      PipelineRuntimeDescription& runtime_description_out);
  // Cheap (no-translation) pre-check used to decide whether a color draw should
  // be configured synchronously so it can take the native XenosRecomp shader
  // path. This keeps the depth pre-pass (always synchronous, native VS-only) and
  // the color pass on the *same* native vertex shader, so both produce identical
  // depth. The authoritative decision is still made by
  // TryConfigureNx1NativePipeline once the shaders are translated.
  bool ShouldForceSyncForNx1NativeColor(
