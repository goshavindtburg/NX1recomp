...
class CommandProcessor {
 public:
  ...
  // When disabled, the worker still executes the ring in full -- fences, EDRAM
  // writebacks and register writes must keep advancing or the guest deadlocks --
  // but it does NOT present (ExecutePacketType3_XE_SWAP skips IssueSwap). Used
  // when a different renderer owns the output, e.g. the native D3D9 path
  // (nx1_d3d9): the D3D9 hooks already suppress the guest's draw and swap PM4, so
  // Xenia processes little more than sync packets and never touches the window.
  void SetGpuDisabled(bool disabled) { gpu_disabled_.store(disabled); }
  bool gpu_disabled() const { return gpu_disabled_.load(); }
 ...
 protected:
  ...
  std::atomic<bool> gpu_disabled_{false};
