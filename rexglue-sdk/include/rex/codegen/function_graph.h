...
class FunctionGraph {
 public:
  ...
  TargetKind classifyTarget(uint32_t target, uint32_t callerAddr, bool isCallInstruction,
                            uint32_t currentFunctionBase = 0) const;
 ...
 private:
  ...
  std::unordered_map<uint32_t, uint32_t> chunkParents_;
  MemoryReader memoryReader_;

  uint32_t chunkParentForAddress(uint32_t address) const;
  bool isAddressInChunkOfFunction(uint32_t address, uint32_t functionBase) const;
