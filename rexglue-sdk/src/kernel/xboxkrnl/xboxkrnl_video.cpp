...
void VdSwap_entry(mapped_void buffer_ptr,      // ptr into primary ringbuffer
  ...
  if (REXCVAR_GET(nx1_gpu_probe_logging) &&
      (nx1_swap_probe_count <= 64 || (nx1_swap_probe_count % 120) == 0)) {
    REXKRNL_WARN(
        "[NX1 swap-probe] VdSwap #{} fetch_ptr=0x{:08X} frontbuffer_v=0x{:08X} "
        "frontbuffer_p=0x{:08X} ptr_v=0x{:08X} fmt={} color_space={} size={}x{} "
        "fetch={:08X} {:08X} {:08X} {:08X} {:08X} {:08X}",
        nx1_swap_probe_count, fetch_ptr.guest_address(), frontbuffer_virtual_address,
        frontbuffer_physical_address, frontbuffer_ptr.value(), texture_format_ptr.value(),
        color_space_ptr.value(), width.value(), height.value(), gpu_fetch.dword_0,
        gpu_fetch.dword_1, gpu_fetch.dword_2, gpu_fetch.dword_3, gpu_fetch.dword_4,
        gpu_fetch.dword_5);
  }
