...
u32 MmQueryStatistics_entry(ppc_ptr_t<X_MM_QUERY_STATISTICS_RESULT> stats_ptr) {
  ...
  uint32_t executable_image_pages = 0;
  auto executable_module = REX_KERNEL_STATE()->GetExecutableModule();
  if (executable_module) {
    auto* executable_xex = executable_module->xex_module();
    const uint32_t loader_image_size = executable_xex->image_size();
    uint32_t executable_accounting_bytes = executable_xex->xex_security_info()->image_size;
    if (!executable_accounting_bytes) {
      executable_accounting_bytes = loader_image_size;
    }

    const uint32_t base_region = executable_xex->base_address() & 0xFF000000u;
    constexpr uint32_t kNx1ExeSizeGlobalDelta = 0x1C1C;
    const uint32_t min_expected_executable_size =
        loader_image_size > 0x00400000u ? loader_image_size - 0x00400000u : 0;
    uint32_t engine_executable_size = 0;
    for (const auto& section : executable_xex->pe_sections()) {
      if ((section.flags & kXEPESectionMemoryWrite) == 0 ||
          section.size < kNx1ExeSizeGlobalDelta + sizeof(uint32_t)) {
        continue;
      }

      const auto* section_data = REX_KERNEL_MEMORY()->TranslateVirtual<const uint8_t*>(
          section.address);
      const uint32_t last_offset = section.size - kNx1ExeSizeGlobalDelta - sizeof(uint32_t);
      for (uint32_t offset = 0; offset <= last_offset; offset += sizeof(uint32_t)) {
        const uint32_t maybe_base = rex::memory::load_and_swap<uint32_t>(section_data + offset);
        if (maybe_base != base_region) {
          continue;
        }

        const uint32_t maybe_size = rex::memory::load_and_swap<uint32_t>(
            section_data + offset + kNx1ExeSizeGlobalDelta);
        if (maybe_size >= min_expected_executable_size && maybe_size <= loader_image_size &&
            maybe_size > engine_executable_size) {
          engine_executable_size = maybe_size;
        }
      }
    }
    if (engine_executable_size) {
      executable_accounting_bytes = engine_executable_size;
    }

    executable_image_pages = rex::round_up(executable_accounting_bytes, 0x1000u) / 0x1000u;
  }

  constexpr uint32_t kXenonExpectedReservedMemory = 0x024DB000;
  const uint32_t expected_reserved_pages =
      rex::round_up(kXenonExpectedReservedMemory, 0x1000u) / 0x1000u;
  const uint32_t max_title_used_pages = stats_ptr->total_physical_pages - stats_ptr->kernel_pages;

  uint32_t accounted_used_pages = used_pages + executable_image_pages;
  if (executable_image_pages) {
    const uint32_t target_boot_used_pages = executable_image_pages + expected_reserved_pages;
    if (target_boot_used_pages > stats_ptr->kernel_pages) {
      accounted_used_pages =
          std::max(accounted_used_pages, target_boot_used_pages - stats_ptr->kernel_pages);
    }
  }
  accounted_used_pages = std::min(accounted_used_pages, max_title_used_pages);

  assert_true(accounted_used_pages < stats_ptr->total_physical_pages);

  stats_ptr->title.available_pages =
      stats_ptr->total_physical_pages - stats_ptr->kernel_pages - accounted_used_pages;
  ...
  stats_ptr->title.image_pages = std::max(executable_image_pages, 0x00000100u);
