...
constexpr uint32_t XINPUT_DEVICE_MASK = 0xFF;
...
bool HasUnsupportedDeviceFlags(uint32_t flags, uint32_t supported_devices) {
  const uint32_t requested_devices = flags & XINPUT_DEVICE_MASK;
  return requested_devices && (requested_devices & ~supported_devices) != 0;
}
...
u32 XamInputGetCapabilities_entry(u32 user_index, u32 flags, ppc_ptr_t<X_INPUT_CAPABILITIES> caps) {
  ...
  if (HasUnsupportedDeviceFlags(flags, XINPUT_FLAG_GAMEPAD | XINPUT_FLAG_KEYBOARD)) {
...
u32 XamInputGetState_entry(u32 user_index, u32 flags, ppc_ptr_t<X_INPUT_STATE> input_state) {
  ...
  if (HasUnsupportedDeviceFlags(flags, XINPUT_FLAG_GAMEPAD)) {
    // Ignore any query for other types of devices.
    return X_ERROR_DEVICE_NOT_CONNECTED;
  }
