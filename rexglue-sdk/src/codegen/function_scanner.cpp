...
FunctionBlocks FunctionScanner::discover_blocks(rex::guest_addr_t entry_point,
  ...
  while (!block_stack.empty() && result.blocks.size() < MAX_BLOCKS) {
    ...
    if (addr != entry_point && known_callables_.contains(static_cast<uint32_t>(addr)) &&
        !isWithinChunk(static_cast<uint32_t>(addr), static_cast<uint32_t>(entry_point))) {
    ...
    if (instr.opcode == Opcode::b || instr.opcode == Opcode::ba) {
      if (instr.branch_target.has_value()) {
        ...
        const bool target_is_chunk =
            isWithinChunk(static_cast<uint32_t>(target), static_cast<uint32_t>(entry_point));
        bool is_tail_call =
            known_callables_.contains(static_cast<uint32_t>(target)) && !target_is_chunk;
        ...
        if (!is_tail_call && !target_is_chunk && isKnownCallable(static_cast<uint32_t>(target))) {
          is_tail_call = true;
        }
        ...
        if (!is_tail_call && !target_is_chunk && is_prologue_pattern(target)) {
...
BlockDiscoveryResult discoverBlocks(DecodedBinary& decoded, uint32_t entryPoint,
                                    ...
                                    const std::unordered_set<uint32_t>& knownFunctions,
                                    uint32_t pdataSize, bool strictFunctionBounds,
                                    const std::unordered_map<uint32_t, FunctionConfig>* chunks) {
  ...
  auto chunkRangeFor = [&](uint32_t addr) -> std::optional<std::pair<uint32_t, uint32_t>> {
    if (!chunks)
      return std::nullopt;
    for (const auto& [chunkStart, cfg] : *chunks) {
      if (cfg.parent != entryPoint)
        continue;
      uint32_t chunkEnd =
          cfg.end ? cfg.end : (cfg.size ? chunkStart + cfg.size : chunkStart + 0x1000);
      if (addr >= chunkStart && addr < chunkEnd) {
        return std::make_pair(chunkStart, chunkEnd);
      }
    }
    return std::nullopt;
  };

  // Helper to check if address is within function bounds or an explicit chunk.
  auto isWithinFunction = [&](uint32_t addr) -> bool {
    return (addr >= entryPoint && addr < funcEnd) || chunkRangeFor(addr).has_value();
  };
  ...
  while (!worklist.empty()) {
    ...
    while (isWithinFunction(addr)) {
      ...
      if (isBranch(*insn)) {
        ...
        auto isLikelyLocalLabel = [&](uint32_t t) -> bool {
          const auto* targetInsn = decoded.get(t);
          return targetInsn && containingRegion.contains(t) && !isProloguePattern(*targetInsn);
        };

        auto isInternalTarget = [&](uint32_t t, bool allowRegionLabel = false) -> bool {
          // Must be within function bounds unless a non-call branch targets a same-region label.
          auto chunkRange = chunkRangeFor(t);
          if ((t < entryPoint || t >= funcEnd) && !chunkRange) {
            if (!allowRegionLabel || !isLikelyLocalLabel(t)) {
              return false;
            }

            funcEnd = containingRegion.end;
          }
          // Must not be a known function entry (except our own entry point, a configured chunk,
          // or a same-region non-prologue label reached by a non-call branch).
          if (t != entryPoint && knownFunctions.contains(t) && !chunkRange) {
            if (!allowRegionLabel || !isLikelyLocalLabel(t)) {
              return false;
            }
          }
        ...
        if (isCall(*insn)) {
          ...
          if (jt) {
            if (strictFunctionBounds) {
              const bool hasOutOfBoundsTarget =
                  std::any_of(jt->targets.begin(), jt->targets.end(), [&](uint32_t t) {
                    return t != 0 && !isWithinFunction(t);
                  });
              if (hasOutOfBoundsTarget) {
                REXCODEGEN_TRACE(
                    "discoverBlocks: discarding jump table at 0x{:08X} for strict function "
                    "0x{:08X}; at least one target exceeds funcEnd=0x{:08X}",
                    addr, entryPoint, funcEnd);
                block.size = addr - blockStart + 4;
                break;
              }
            ...
            // Jump table targets are normally part of this function. For explicit config ranges,
            // don't let a misdetected or oversized table merge neighboring functions.
            for (uint32_t t : jt->targets) {
              if (t == 0)
                continue;  // sentinel from null-padding detection
              if (strictFunctionBounds && !isWithinFunction(t)) {
                REXCODEGEN_TRACE(
                    "discoverBlocks: strict bounds skip jump-table target 0x{:08X} for function "
                    "0x{:08X} (funcEnd=0x{:08X})",
                    t, entryPoint, funcEnd);
                continue;
              }
          ...
          if (target && isInternalTarget(*target, true)) {
          ...
          // Unconditional branch
          if (target) {
            if (isInternalTarget(*target, true)) {
      ...
      // Some Xbox 360 linkers expose a symbol at a shared tail that is also
      // reached by plain fallthrough from the previous symbol. A C++ function
      // ending at that boundary would silently return early, leaving the PPC
      // stack/register frame unrestored. If the next configured entry is not a
      // prologue, treat it as a local continuation label for this path.
      uint32_t fallthrough = addr + 4;
      if (fallthrough == funcEnd && fallthrough < containingRegion.end &&
          knownFunctions.contains(fallthrough)) {
        auto* fallthroughInsn = decoded.get(fallthrough);
        if (fallthroughInsn && !isProloguePattern(*fallthroughInsn)) {
          REXCODEGEN_TRACE(
              "discoverBlocks: 0x{:08X} extends through fallthrough shared tail 0x{:08X}",
              entryPoint, fallthrough);
          result.labels.insert(fallthrough);
          funcEnd = containingRegion.end;
        }
      }
