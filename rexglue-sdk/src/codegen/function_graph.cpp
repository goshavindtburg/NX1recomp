          ...
          if (is_switch_pattern) {
            ...
            if (jt_opt.has_value()) {
              const bool hasOutOfBoundsTarget =
                  std::any_of(jt_opt->targets.begin(), jt_opt->targets.end(), [&](uint32_t t) {
                    return t != 0 && (t < base() || t >= end());
                  });
              if (hasOutOfBoundsTarget) {
                REXCODEGEN_TRACE(
                    "Discarding late-detected jump table at 0x{:08X} for function 0x{:08X}; "
                    "target outside function end 0x{:08X}",
                    blockBase, base(), end());
                continue;
              }
...
FunctionNode* FunctionGraph::addFunction(uint32_t base, uint32_t size, FunctionAuthority authority,
  ...
  if (node && !name.empty() && node->authority() <= authority) {
...
void FunctionGraph::addUnresolvedJumpToFunction(uint32_t entry, uint32_t site, uint32_t target,
  ...
  if (isAddressInChunkOfFunction(target, entry)) {
    node->addLabel(target);
    REXCODEGEN_TRACE("FunctionGraph: resolved 0x{:08X}->0x{:08X} as parent chunk label", site,
                     target);
    return;
  }
...
size_t FunctionGraph::tryResolveFunction(uint32_t entry) {
  ...
  for (const auto& jump : jumps) {
    if (isAddressInChunkOfFunction(jump.target, entry)) {
...
uint32_t FunctionGraph::chunkParentForAddress(uint32_t address) const {
  for (const auto& chunk : chunks_) {
    if (chunk.parent != 0 && address >= chunk.base && address < chunk.base + chunk.size) {
      return chunk.parent;
    }
  }
  return 0;
...
bool FunctionGraph::isAddressInChunkOfFunction(uint32_t address, uint32_t functionBase) const {
  return chunkParentForAddress(address) == functionBase;
}
...
TargetKind FunctionGraph::classifyTarget(uint32_t target, uint32_t callerAddr,
                                         ...
                                         uint32_t currentFunctionBase) const {
  // Find the caller's function
  const FunctionNode* callerFn =
      currentFunctionBase != 0 ? getFunction(currentFunctionBase) : getFunctionContaining(callerAddr);
  ...
  // Configured chunks are entry points for external callers, but branches from
  // their configured parent should remain local control flow.
  if (uint32_t chunkParent = chunkParentForAddress(target); chunkParent != 0) {
    if (currentFunctionBase != 0 && currentFunctionBase == chunkParent) {
      return TargetKind::InternalLabel;
    }
    if (currentFunctionBase == 0) {
      if (const auto* parentFn = getFunction(chunkParent);
          parentFn && parentFn->containsAddress(callerAddr)) {
        return TargetKind::InternalLabel;
      }
    }
    if (callerFn && callerFn->base() == chunkParent) {
      return TargetKind::InternalLabel;
    }
  }

  // Case 3: A non-call branch to an address already discovered inside the caller is local
  // control flow, even when approximate map data also marks that address as an entry point.
  if (!isCallInstruction && callerFn && callerFn->containsAddress(target) &&
      callerFn->isLabel(target)) {
    return TargetKind::InternalLabel;
  }
