...
bool CodegenWriter::write(bool force) {
  ...
  const uint32_t imageBase = ctx_.binary().baseAddress();
  const uint32_t imageEnd = imageBase + ctx_.binary().imageSize();
  std::erase_if(functions, [imageBase, imageEnd](const FunctionNode* fn) {
    const uint32_t base = static_cast<uint32_t>(fn->base());
    if (fn->authority() == FunctionAuthority::IMPORT) {
      return false;
    }

    if (base >= imageBase && base < imageEnd) {
      return false;
    }

    REXCODEGEN_WARN("Recompile: skipping function 0x{:08X} outside image range 0x{:08X}-0x{:08X}",
                    base, imageBase, imageEnd);
    return true;
  });
