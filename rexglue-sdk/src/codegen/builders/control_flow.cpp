...
bool build_b(BuilderContext& ctx) {
  ...
  auto kind = ctx.graph().classifyTarget(target, ctx.base, false, ctx.fn.base());
...
bool build_bl(BuilderContext& ctx) {
  ...
  auto kind = ctx.graph().classifyTarget(target, ctx.base, true, ctx.fn.base());
...
bool build_bctr(BuilderContext& ctx) {
  ...
  if (jt) {
    ...
    for (size_t i = 0; i < jt->targets.size(); i++) {
      ...
      auto kind = ctx.graph().classifyTarget(label, ctx.base, false, ctx.fn.base());
      switch (kind) {
        case TargetKind::InternalLabel:
