...
void discoverFunction(CodegenContext& ctx, uint32_t funcAddr,
  ...
  const bool strictFunctionBounds =
      node->authority() == FunctionAuthority::CONFIG && node->size() > 0;

  // Pass pdataSize so forward branches within function extent are correctly identified.
  // Configured function sizes come from user-supplied symbol data, so keep them as hard bounds.
  auto result =
      discoverBlocks(decoded, funcAddr, *region, knownFunctions, pdataSize, strictFunctionBounds,
