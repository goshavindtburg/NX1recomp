...
bool isSaveRestoreHelperName(std::string_view name) {
  return name.find("savegprlr") != std::string_view::npos ||
         name.find("restgprlr") != std::string_view::npos ||
         name.find("savefpr") != std::string_view::npos ||
         name.find("restfpr") != std::string_view::npos ||
         name.find("savevmx") != std::string_view::npos ||
         name.find("restvmx") != std::string_view::npos;
}
...
VoidResult registerEntryPoints(CodegenContext& ctx) {
  ...
  for (const auto& [address, cfg] : config.functions) {
    ...
    // The compiler save/restore helper tables intentionally fall through from
    // one register label to the next. Symbol maps often expose each label as a
    // 4-byte function, which would truncate the generated helper and skip the
    // later register saves/restores.
    auto* existing = graph.getFunction(address);
    if (existing && existing->isHelper() && isSaveRestoreHelperName(name)) {
    ...
    if (node && node->authority() <= FunctionAuthority::CONFIG) {
      graph.setFunctionName(address, name);
    }
    configFuncs++;
