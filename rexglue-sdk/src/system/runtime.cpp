...
X_STATUS Runtime::Setup(RuntimeConfig config) {
  ...
  setup_complete_ = true;
  system::DiscordPresenceClient::Get().SetMenuState(kernel_state_.get());
  return X_STATUS_SUCCESS;
}
...
void Runtime::Shutdown() {
  ...
  if (setup_complete_ && kernel_state_) {
    // Stop and drain the System Link broadcast bridge's background refresh
    // threads before anything they touch (KernelState, the web client) is torn
    // down below.
    system::ShutdownBroadcastBridge();
    system::XLiveWebClient::Get().CleanupOnShutdown(kernel_state_.get());
    system::DiscordPresenceClient::Get().Shutdown();
  }
