...
static void InvalidFunctionTrap(PPCContext& ctx, uint8_t* /*base*/) {
  if (auto* runtime = Runtime::instance()) {
    if (auto* memory = runtime->memory()) {
      uint32_t slot = ctx.r30.u32;
      if (slot >= 0x30000000 && slot < 0x90000000) {
        uint32_t start = (slot >= 0x40) ? (slot - 0x40) : slot;
        uint32_t end = slot + 0x40;
        for (uint32_t entry = start; entry <= end; entry += 4) {
          uint32_t target = memory::load_and_swap<uint32_t>(memory->TranslateVirtual(entry));
          if (target < 0x82000000 || target >= 0x82600000) {
            continue;
          }
          REXLOG_ERROR("InvalidFunctionTrap: table[0x{:08X}] = 0x{:08X} ({})", entry, target,
                       runtime->function_dispatcher()->GetFunction(target) ? "registered"
                                                                            : "unregistered");
        }
      }
    }
  ...
  REX_FATAL(
      ...
      "(lr=0x{:08X}, ctr=0x{:08X}, r11=0x{:08X}, r12=0x{:08X}, "
      "r29=0x{:08X}, r30=0x{:08X}, r31=0x{:08X})",
      ctx.last_indirect_target, static_cast<uint32_t>(ctx.lr), ctx.ctr.u32, ctx.r11.u32,
      ctx.r12.u32, ctx.r29.u32, ctx.r30.u32, ctx.r31.u32);
}

static void NoOpReturnTrap(PPCContext& /*ctx*/, uint8_t* /*base*/) {}
...
PPCFunc* ResolveIndirectFunction(uint32_t guest_address) {
  ...
  if (memory) {
    uint32_t instruction =
        memory::load_and_swap<uint32_t>(memory->TranslateVirtual(guest_address));
    if (instruction == 0x4E800020) {  // blr
      REXCPU_TRACE("ResolveIndirectFunction({:08X}): treating unregistered blr as no-op",
                   guest_address);
      return &NoOpReturnTrap;
    }
  }
