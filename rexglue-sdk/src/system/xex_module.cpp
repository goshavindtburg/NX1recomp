...
int XexModule::ReadImageBasicCompressed(const void* xex_addr, size_t xex_length) {
  ...
  const uint8_t* source_end = source_buffer + exe_length;
  ...
  REXLOG_INFO(
      "XEX basic compression: exe=0x{:X}, blocks={}, image=0x{:X}, allocation=0x{:X}, "
      "encryption={}",
      exe_length, block_count, uncompressed_size, total_size,
      static_cast<uint32_t>(opt_file_format_info()->encryption_type));
  ...
  uint8_t* d = buffer;
  uint8_t* buffer_end = buffer + total_size;
  ...
  for (size_t n = 0; n < block_count; n++) {
    ...
    if (p + data_size > source_end) {
      REXLOG_ERROR(
          "XEX basic compression block {} reads past file data: data=0x{:X}, remaining=0x{:X}, "
          "exe=0x{:X}",
          n, data_size, static_cast<uint32_t>(source_end - p), exe_length);
      return 4;
    }
    if (d + data_size + zero_size > buffer_end) {
      REXLOG_ERROR(
          "XEX basic compression block {} writes past image allocation: data=0x{:X}, zero=0x{:X}, "
          "remaining=0x{:X}, total=0x{:X}, expected=0x{:X}",
          n, data_size, zero_size, static_cast<uint32_t>(buffer_end - d), total_size,
          uncompressed_size);
      return 5;
    }
    if (opt_file_format_info()->encryption_type == XEX_ENCRYPTION_NORMAL && (data_size & 0xF)) {
      REXLOG_ERROR("XEX basic compression block {} has unaligned encrypted data size 0x{:X}", n,
                   data_size);
      return 6;
    }
    REXLOG_INFO("XEX basic block {}: src=0x{:X}/0x{:X}, dst=0x{:X}/0x{:X}, data=0x{:X}, zero=0x{:X}",
                n, static_cast<uint32_t>(p - source_buffer), exe_length,
                static_cast<uint32_t>(d - buffer), total_size, data_size, zero_size);
    ...
    REXLOG_INFO("XEX basic block {} complete", n);
  }
