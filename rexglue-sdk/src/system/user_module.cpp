...
X_STATUS UserModule::LoadXexContinue() {
  ...
  uint32_t xex_stack_size = 0;
  this->xex_module()->GetOptHeader(XEX_HEADER_DEFAULT_STACK_SIZE, &xex_stack_size);
  stack_size_ = xex_stack_size;
  if (stack_size_ < 4 * 1024 * 1024) {
    REXSYS_INFO("XEX default stack 0x{:X}; clamping recompiled main stack to 4 MiB",
                stack_size_);
    stack_size_ = 4 * 1024 * 1024;
  }
  ...
  ldr_data->image_size = security_header->image_size;
