...
void KernelState::SetProcessTLSVars(X_KPROCESS* process, uint32_t num_slots, uint32_t tls_data_size,
  ...
  // Slot 0 is reserved by several Xbox 360-era runtime libraries as an
  // invalid/sentinel value; dynamic allocations start at slot 1.
  process->tls_slot_bitmap[0] = static_cast<uint32_t>(process->tls_slot_bitmap[0]) & ~(1u << 31);
}
...
void KernelState::SetExecutableModule(object_ref<UserModule> module) {
  ...
  // NX1 distinguishes SP/MP very early by the launch executable name rather
  // than only by the loaded image path.
  export_entry = emulator_->function_dispatcher()->export_resolver()->GetExportByOrdinal(
      "xboxkrnl.exe", 0x01AE /* ExLoadedCommandLine */);
  if (export_entry) {
    char* variable_ptr = memory_->TranslateVirtual<char*>(export_entry->variable_ptr);
    auto image_name = rex::string::utf8_lower_ascii(
        rex::string::utf8_find_name_from_guest_path(executable_module_->path()));
    const char* launch_name =
        (image_name.find("mp") != std::string::npos) ? "default_mp.xex" : "default.xex";
    std::string command_line = fmt::format("\"{}\"", launch_name);
    if (!REXCVAR_GET(cl).empty()) {
      command_line += " " + REXCVAR_GET(cl);
    }
    std::memset(variable_ptr, 0, 1024);
    rex::string::util_copy_truncating(variable_ptr, command_line, 1024);
    REXSYS_INFO("ExLoadedCommandLine: {}", command_line);
  }
  ...
  if (!dispatch_thread_running_) {
    ...
    dispatch_thread_ = object_ref<XHostThread>(new XHostThread(this, 128 * 1024, 0, [this]() {
      ...
      REXSYS_DEBUG("Kernel dispatch thread started");
      while (dispatch_thread_running_) {
        global_lock.lock();
        dispatch_cond_.wait(global_lock, [this]() {
          return !dispatch_thread_running_ || !dispatch_queue_.empty();
        });
        if (!dispatch_thread_running_) {
          global_lock.unlock();
          break;
