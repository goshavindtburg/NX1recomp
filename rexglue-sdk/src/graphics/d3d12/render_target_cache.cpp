...
REXCVAR_DEFINE_BOOL(nx1_probe_scene_composite_host_rt, false, "GPU/D3D12",
                    "NX1 diagnostic: sample the host render target used by the scene composite "
                    "resolve before it is dumped to EDRAM")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DECLARE(bool, nx1_gpu_probe_logging);
...
bool D3D12RenderTargetCache::Resolve(const memory::Memory& memory, D3D12SharedMemory& shared_memory,
  ...
  static uint64_t nx1_resolve_probe_count = 0;
  ++nx1_resolve_probe_count;
  bool nx1_log_resolve_probe = REXCVAR_GET(nx1_gpu_probe_logging) &&
                               (nx1_resolve_probe_count <= 128 ||
                                (nx1_resolve_probe_count % 500) == 0);
  if (nx1_log_resolve_probe) {
    REXGPU_WARN(
        "[NX1 resolve-probe] resolve #{} rb_copy_control=0x{:08X} copy_dest_info=0x{:08X} "
        "copy_src={} copy_dest_base=0x{:08X} extent=0x{:08X}+0x{:X} coord_width_div8={} "
        "height_div8={} clear_depth={} clear_color={} draw_scaled={}",
        nx1_resolve_probe_count, resolve_info.rb_copy_control.value,
        resolve_info.copy_dest_info.value, uint32_t(resolve_info.rb_copy_control.copy_src_select),
        resolve_info.copy_dest_base, resolve_info.copy_dest_extent_start,
        resolve_info.copy_dest_extent_length,
        uint32_t(resolve_info.coordinate_info.width_div_8), resolve_info.height_div_8,
        resolve_info.IsClearingDepth(), resolve_info.IsClearingColor(), draw_resolution_scaled);
  }
      ...
      if (GetPath() == Path::kHostRenderTargets) {
        ...
        if (nx1_log_resolve_probe) {
          ...
          resolve_info.GetCopyEdramTileSpan(dump_base, dump_row_length_used, dump_rows,
                                            dump_pitch);
          REXGPU_WARN(
              "[NX1 resolve-probe] resolve #{} copy shader={} groups={}x{} dest_bpe_log2={} "
              "source_bpe_log2={} source_raw={} direct_resolved={} edram_base=0x{:X} "
              "edram_rows={} row_used={} pitch={}",
              nx1_resolve_probe_count, uint32_t(copy_shader), copy_group_count_x,
              copy_group_count_y, copy_shader_info.dest_bpe_log2,
              copy_shader_info.source_bpe_log2, copy_shader_info.source_is_raw, direct_resolved,
  ...
  if (nx1_log_resolve_probe) {
    REXGPU_WARN("[NX1 resolve-probe] resolve #{} result copied={} cleared={} wrote=0x{:08X}+0x{:X}",
                nx1_resolve_probe_count, copied, cleared, written_address_out,
                written_length_out);
  }
...
bool D3D12RenderTargetCache::DumpRenderTargets(uint32_t dump_base, uint32_t dump_row_length_used,
  ...
  struct Nx1SceneCompositeHostReadback {
    bool valid = false;
  ...
  static uint64_t nx1_scene_dump_count = 0;
  static Nx1SceneCompositeHostReadback nx1_scene_readback;

  const bool nx1_probe_scene_host_rt = REXCVAR_GET(nx1_probe_scene_composite_host_rt);
  const bool nx1_scene_composite_dump =
      nx1_probe_scene_host_rt && dump_base == 0x62 && dump_pitch == 13 &&
      dump_row_length_used == 13 && dump_rows == 75;
  bool nx1_log_scene_dump = false;
  if (nx1_scene_composite_dump) {
    ++nx1_scene_dump_count;
    nx1_log_scene_dump =
        nx1_scene_dump_count <= 8 || (nx1_scene_dump_count % 240) == 0;
  }

  if (nx1_probe_scene_host_rt && nx1_scene_readback.valid &&
      command_processor_.GetCompletedSubmission() >= nx1_scene_readback.submission) {
    D3D12_RANGE read_range = {0, size_t(nx1_scene_readback.total_size)};
    void* mapping = nullptr;
    if (SUCCEEDED(nx1_scene_readback.buffer->Map(0, &read_range, &mapping))) {
      const uint8_t* bytes = static_cast<const uint8_t*>(mapping);
      const uint32_t sample_width = std::min(nx1_scene_readback.width, 1024u);
      const uint32_t sample_height = std::min(nx1_scene_readback.height, 600u);
      const uint64_t sample_count = uint64_t(sample_width) * sample_height;
      const bool can_sample_words =
          sample_width && sample_height &&
          nx1_scene_readback.row_size >= uint64_t(sample_width) * sizeof(uint32_t);
      ...
      const uint32_t color_mask =
          nx1_scene_readback.format == DXGI_FORMAT_R10G10B10A2_UNORM ? 0x3FFFFFFFu
                                                                      : 0x00FFFFFFu;
      auto sample_word = [&](uint32_t x, uint32_t y) -> uint32_t {
        if (!can_sample_words || x >= sample_width || y >= sample_height) {
          return 0;
        }
        uint32_t word = 0;
        std::memcpy(&word,
                    bytes + nx1_scene_readback.footprint.Offset +
                        uint64_t(y) * nx1_scene_readback.footprint.Footprint.RowPitch +
                        uint64_t(x) * sizeof(uint32_t),
                    sizeof(word));
        return word;
      };
      auto max_rgb8 = [&](uint32_t word) -> uint32_t {
        if (nx1_scene_readback.format == DXGI_FORMAT_R10G10B10A2_UNORM) {
      ...
      nx1_scene_readback.buffer->Unmap(0, &write_range);
      REXGPU_WARN(
          "[NX1 scene-host-rt] dump #{} key=0x{:08X} base={} pitch={} fmt={} depth={} "
          "msaa={} dxgi={} size={}x{} sample={}x{} row_pitch={} color_nonzero={}/{} "
          "rgb_gt8={} rgb_gt32={} rgb_gt64={} min=0x{:08X} max=0x{:08X} center=0x{:08X} "
          "q1=0x{:08X} q3=0x{:08X} first=({}, {}) 0x{:08X}",
          nx1_scene_readback.dump_index, nx1_scene_readback.key.key,
          uint32_t(nx1_scene_readback.key.base_tiles), nx1_scene_readback.key.GetPitchTiles(),
          uint32_t(nx1_scene_readback.key.resource_format),
          uint32_t(nx1_scene_readback.key.is_depth),
          uint32_t(1) << uint32_t(nx1_scene_readback.key.msaa_samples),
          uint32_t(nx1_scene_readback.format), nx1_scene_readback.width,
          nx1_scene_readback.height, sample_width, sample_height,
          nx1_scene_readback.footprint.Footprint.RowPitch, color_nonzero, sample_count,
          rgb_gt8, rgb_gt32, rgb_gt64, min_color == UINT32_MAX ? 0 : min_color, max_color,
          sample_word(sample_width / 2, sample_height / 2),
          sample_word(sample_width / 4, sample_height / 4),
          sample_word((sample_width * 3) / 4, (sample_height * 3) / 4),
          first_nonzero_x == UINT32_MAX ? 0 : first_nonzero_x,
          first_nonzero_y == UINT32_MAX ? 0 : first_nonzero_y, first_nonzero_word);
    } else {
      REXGPU_WARN("[NX1 scene-host-rt] dump #{} readback map failed",
                  nx1_scene_readback.dump_index);
    }
    nx1_scene_readback = {};
  }
  ...
  GetResolveCopyRectanglesToDump(dump_base, dump_row_length_used, dump_rows, dump_pitch,
                                 dump_rectangles_);
  if (nx1_log_scene_dump) {
    REXGPU_WARN("[NX1 scene-dump] dump #{} base=0x{:X} row_used={} rows={} pitch={} rects={}",
                nx1_scene_dump_count, dump_base, dump_row_length_used, dump_rows, dump_pitch,
                dump_rectangles_.size());
    for (uint32_t i = 0; i < std::min<uint32_t>(uint32_t(dump_rectangles_.size()), 8u); ++i) {
      const ResolveCopyDumpRectangle& rectangle = dump_rectangles_[i];
      const auto* d3d12_rt = static_cast<const D3D12RenderTarget*>(rectangle.render_target);
      RenderTargetKey rt_key = d3d12_rt ? d3d12_rt->key() : RenderTargetKey();
      REXGPU_WARN(
          "[NX1 scene-dump] rect {} key=0x{:08X} base={} pitch={} fmt={} depth={} msaa={} "
          "row_first={} rows={} first_start={} last_end={}",
          i, rt_key.key, uint32_t(rt_key.base_tiles), rt_key.GetPitchTiles(),
          uint32_t(rt_key.resource_format), uint32_t(rt_key.is_depth),
          uint32_t(1) << uint32_t(rt_key.msaa_samples), rectangle.row_first, rectangle.rows,
          rectangle.row_first_start, rectangle.row_last_end);
    }
  }
  ...
  DeferredCommandList& command_list = command_processor_.GetDeferredCommandList();
  if (nx1_log_scene_dump && !nx1_scene_readback.valid) {
    for (const ResolveCopyDumpRectangle& rectangle : dump_rectangles_) {
      auto& d3d12_rt = *static_cast<D3D12RenderTarget*>(rectangle.render_target);
      RenderTargetKey rt_key = d3d12_rt.key();
      if (rt_key.is_depth || rt_key.base_tiles != dump_base || rt_key.GetPitchTiles() != dump_pitch) {
        continue;
      }
      ...
      if (resource_desc.Dimension != D3D12_RESOURCE_DIMENSION_TEXTURE2D ||
        ...
        REXGPU_WARN(
            "[NX1 scene-host-rt] dump #{} skipped resource dimension={} samples={} size={}x{}",
            nx1_scene_dump_count, uint32_t(resource_desc.Dimension),
            resource_desc.SampleDesc.Count, uint32_t(resource_desc.Width),
            uint32_t(resource_desc.Height));
        break;
      }
      Nx1SceneCompositeHostReadback readback;
      readback.dump_index = nx1_scene_dump_count;
      readback.width = uint32_t(resource_desc.Width);
      readback.height = uint32_t(resource_desc.Height);
      ...
      if (!readback.total_size || !readback.num_rows || !readback.row_size) {
        REXGPU_WARN("[NX1 scene-host-rt] dump #{} skipped empty footprint",
                    nx1_scene_dump_count);
        break;
      }
      ...
      if (FAILED(device->CreateCommittedResource(
        ...
        REXGPU_WARN("[NX1 scene-host-rt] dump #{} readback allocation failed",
                    nx1_scene_dump_count);
        break;
      }
      command_processor_.PushTransitionBarrier(
          resource, d3d12_rt.SetResourceState(D3D12_RESOURCE_STATE_COPY_SOURCE),
      ...
      command_list.D3DCopyTextureRegion(&copy_dest, 0, 0, 0, &copy_source, nullptr);
      command_processor_.PushTransitionBarrier(
          resource, d3d12_rt.SetResourceState(D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE),
          D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
      readback.submission = command_processor_.GetCurrentSubmission();
      readback.valid = true;
      nx1_scene_readback = std::move(readback);
      break;
    }
  }
