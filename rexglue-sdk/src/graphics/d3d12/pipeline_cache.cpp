...
PipelineCache::PipelineCache(D3D12CommandProcessor& command_processor,
  ...
  if (edram_rov_used) {
    depth_only_pixel_shader_ = shader_translator_->CreateDepthOnlyPixelShader();
  }
}
...
void PipelineCache::InitializeShaderStorage(const std::filesystem::path& cache_root,
  ...
  if (!pipeline_stored_descriptions.empty()) {
    ...
    for (const PipelineStoredDescription& pipeline_stored_description :
      ...
      // NX1 native pipelines reference XenosRecomp DXIL blobs that are NOT stored
      // in the pipeline storage file, and this reload path does not rebuild them
      // (nor does it assign the native root signature). Creating such a pipeline
      // here would fail (null bytecode) and poison the cache with a dead
      // null-state pipeline that later draws reuse and silently skip. Skip them;
      // they are cheaply recreated at runtime from the precompiled DXIL cache.
      if (pipeline_description.nx1_native_shader_cache) {
        continue;
      }
...
bool PipelineCache::ConfigurePipeline(
  ...
  bool use_async = REXCVAR_GET(async_shader_compilation) && !creation_threads_.empty() &&
                   pixel_shader != nullptr;
  // The async path builds the pipeline description with for_placeholder=true,
  // which skips native XenosRecomp shader selection entirely - so color draws
  // would always fall back to translated DXBC shaders while their matching
  // depth pre-pass (VS-only, always synchronous) uses the native DXIL vertex
  // shader. Those two vertex shaders do not produce identical clip-space depth,
  // so geometry fails the depth test and disappears. When the color draw is
  // eligible for the native path, configure it synchronously so it takes the
  // same native vertex shader as the pre-pass.
  if (use_async && ShouldForceSyncForNx1NativeColor(vertex_shader, pixel_shader)) {
    use_async = false;
  }
  ...
  // Do not persist NX1 native pipelines: the storage file can't carry their
  // XenosRecomp DXIL blobs and the reload path would recreate them as dead
  // null-state pipelines. They are recreated at runtime from the DXIL cache.
  if (pipeline_storage_file_ && !description.nx1_native_shader_cache) {
    assert_not_null(storage_write_thread_);
...
bool PipelineCache::GetCurrentStateDescription(
  ...
  if (!for_placeholder) {
    TryConfigureNx1NativePipeline(vertex_shader, pixel_shader, primitive_processing_result,
                                  normalized_color_mask,
                                  bound_depth_and_color_render_target_bits,
                                  runtime_description_out);
  }

  return true;
}

bool PipelineCache::TryConfigureNx1NativePipeline(
    D3D12Shader::D3D12Translation* vertex_shader, D3D12Shader::D3D12Translation* pixel_shader,
    const PrimitiveProcessor::ProcessingResult& primitive_processing_result,
    uint32_t normalized_color_mask, uint32_t bound_depth_and_color_render_target_bits,
    PipelineRuntimeDescription& runtime_description_out) {
  if (!REXCVAR_GET(nx1_native_shader_cache)) {
    return false;
  }
  auto log_fallback = [](const char* reason, uint64_t vs_hash, uint64_t ps_hash = 0) {
    if (REXCVAR_GET(nx1_native_shader_cache_log)) {
      // Deduplicate per VS/PS/reason to avoid flooding the rotating log.
      static std::mutex nx1_fb_mutex;
      static std::unordered_set<uint64_t> nx1_fb_logged;
      const uint64_t key =
          vs_hash ^ (ps_hash * 0x9E3779B97F4A7C15ull) ^ (std::hash<const char*>{}(reason) << 1);
      std::lock_guard<std::mutex> lock(nx1_fb_mutex);
      if (!nx1_fb_logged.insert(key).second) {
        return;
      }
      if (ps_hash) {
        REXGPU_INFO("[NX1 native shader] fallback: {} VS {:016X} PS {:016X}", reason, vs_hash,
                    ps_hash);
      } else {
        REXGPU_INFO("[NX1 native shader] fallback: {} VS {:016X}", reason, vs_hash);
      }
    }
  };

  const uint64_t vs_hash = vertex_shader->shader().ucode_data_hash();
  const uint64_t ps_hash = pixel_shader ? pixel_shader->shader().ucode_data_hash() : 0;
  if (!bindless_resources_used_) {
    log_fallback("bindless resources are disabled", vs_hash, ps_hash);
    return false;
  }
  ID3D12RootSignature* native_root_signature =
      command_processor_.GetNx1NativeRootSignature();
  if (!native_root_signature) {
    log_fallback("native root signature unavailable", vs_hash, ps_hash);
    return false;
  }
  if (primitive_processing_result.IsTessellated() ||
      DxbcShaderTranslator::Modification(vertex_shader->modification())
              .vertex.host_vertex_shader_type != Shader::HostVertexShaderType::kVertex ||
      runtime_description_out.geometry_shader != nullptr ||
      runtime_description_out.description.geometry_shader != PipelineGeometryShader::kNone) {
    log_fallback("unsupported primitive shader path", vs_hash, ps_hash);
    return false;
  }
  if (vertex_shader->shader().memexport_eM_written() ||
      (pixel_shader && pixel_shader->shader().memexport_eM_written())) {
    log_fallback("memexport draw", vs_hash, ps_hash);
    return false;
  }

  if (!pixel_shader) {
    if (normalized_color_mask) {
      log_fallback("missing pixel shader for color-writing draw", vs_hash);
      return false;
    }
    if (!REXCVAR_GET(nx1_native_shader_cache_vs_only)) {
      log_fallback("VS-only native shader path disabled", vs_hash);
      return false;
    }
  }

  Nx1NativeDxilBlob native_vs;
  Nx1NativeDxilBlob native_ps;
  if (!GetNx1NativeDxilBlob(vs_hash, native_vs) ||
      (pixel_shader && !GetNx1NativeDxilBlob(ps_hash, native_ps))) {
    log_fallback("DXIL cache miss", vs_hash, ps_hash);
    return false;
  }
  constexpr uint32_t kNx1NativeSupportedSpecConstants = (1u << 0) | (1u << 1);
  const uint32_t native_spec_constants_mask =
      native_vs.spec_constants_mask | (pixel_shader ? native_ps.spec_constants_mask : 0);
  if (native_spec_constants_mask & ~kNx1NativeSupportedSpecConstants) {
    log_fallback("unsupported spec-constant mask", vs_hash, ps_hash);
    return false;
  }

  auto bindings_fit_shared_layout = [](const D3D12Shader& shader) {
  ...
  if (!bindings_fit_shared_layout(static_cast<const D3D12Shader&>(vertex_shader->shader())) ||
      (pixel_shader &&
       !bindings_fit_shared_layout(static_cast<const D3D12Shader&>(pixel_shader->shader())))) {
    log_fallback("texture/sampler fetch constant exceeds native shared layout", vs_hash, ps_hash);
    return false;
  }
  ...
  runtime_description_out.root_signature = native_root_signature;
  runtime_description_out.description.nx1_native_shader_cache = 1;
  runtime_description_out.nx1_native_vertex_shader.pShaderBytecode = native_vs.data;
  runtime_description_out.nx1_native_vertex_shader.BytecodeLength = native_vs.size;
  if (pixel_shader) {
    runtime_description_out.nx1_native_pixel_shader.pShaderBytecode = native_ps.data;
    runtime_description_out.nx1_native_pixel_shader.BytecodeLength = native_ps.size;
  }
  runtime_description_out.nx1_native_input_element_count = input_element_count;
  runtime_description_out.nx1_native_vertex_buffer_mask = vertex_buffer_mask;
  if (REXCVAR_GET(nx1_native_shader_cache_log)) {
    // Deduplicate by VS/PS so this per-draw config message doesn't flood the
    // rotating log and push the (deduplicated) diagnostic lines out of the
    // retained chunks.
    static std::mutex nx1_cfg_mutex;
    static std::unordered_set<uint64_t> nx1_cfg_logged;
    const uint64_t key = vs_hash ^ (ps_hash * 0x9E3779B97F4A7C15ull);
    std::lock_guard<std::mutex> lock(nx1_cfg_mutex);
    if (nx1_cfg_logged.insert(key).second) {
      if (pixel_shader) {
        REXGPU_INFO(
            "[NX1 native shader] native PSO VS {:016X} PS {:016X} inputs={} vb_mask=0x{:08X}",
            vs_hash, ps_hash, input_element_count, vertex_buffer_mask);
      } else {
        REXGPU_INFO(
            "[NX1 native shader] native VS-only PSO VS {:016X} inputs={} vb_mask=0x{:08X} "
            "rt_bits=0x{:X} color_mask=0x{:X}",
            vs_hash, input_element_count, vertex_buffer_mask,
            bound_depth_and_color_render_target_bits, normalized_color_mask);
    }
    }
  }
  return true;
...
bool PipelineCache::ShouldForceSyncForNx1NativeColor(
    const D3D12Shader::D3D12Translation* vertex_shader,
    const D3D12Shader::D3D12Translation* pixel_shader) const {
  // Only color draws (those with a pixel shader) are affected; VS-only draws are
  // already configured synchronously.
  if (!pixel_shader) {
    return false;
  }
  if (!REXCVAR_GET(nx1_native_shader_cache) || !REXCVAR_GET(nx1_native_shader_cache_color)) {
    return false;
  }
  if (!bindless_resources_used_) {
    return false;
  }
  if (!command_processor_.GetNx1NativeRootSignature()) {
    return false;
  }
  // Both the vertex and pixel shader must be present in the native DXIL cache,
  // otherwise the draw cannot take the native path and there is no reason to pay
  // the cost of synchronous configuration. This mirrors the cache-hit checks in
  // TryConfigureNx1NativePipeline, which remains the authoritative decision once
  // the shaders are translated (it additionally rejects tessellation, memexport
  // and bindings that don't fit the native shared layout).
  const uint64_t vs_hash = vertex_shader->shader().ucode_data_hash();
  const uint64_t ps_hash = pixel_shader->shader().ucode_data_hash();
  Nx1NativeDxilBlob native_vs;
  Nx1NativeDxilBlob native_ps;
  if (!GetNx1NativeDxilBlob(vs_hash, native_vs) || !GetNx1NativeDxilBlob(ps_hash, native_ps)) {
    return false;
  }
  constexpr uint32_t kNx1NativeSupportedSpecConstants = (1u << 0) | (1u << 1);
  if ((native_vs.spec_constants_mask | native_ps.spec_constants_mask) &
      ~kNx1NativeSupportedSpecConstants) {
    return false;
  }
  return true;
}
...
ID3D12PipelineState* PipelineCache::CreateD3D12Pipeline(
  ...
  if (Shader::IsHostVertexShaderTypeDomain(host_vertex_shader_type)) {
    ...
    if (description.nx1_native_shader_cache) {
      state_desc.VS = runtime_description.nx1_native_vertex_shader;
      state_desc.InputLayout.pInputElementDescs =
          runtime_description.nx1_native_input_elements;
      state_desc.InputLayout.NumElements =
          runtime_description.nx1_native_input_element_count;
    } else {
  ...
  if (runtime_description.pixel_shader != nullptr) {
    ...
    if (description.nx1_native_shader_cache) {
      state_desc.PS = runtime_description.nx1_native_pixel_shader;
    } else {
