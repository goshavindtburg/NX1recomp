...
REXCVAR_DEFINE_BOOL(nx1_force_predicated_pm4, false, "GPU",
                    "NX1 diagnostic: force predicated PM4 packets to execute")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);
...
void CommandProcessor::WorkerThreadMain() {
  ...
  while (worker_running_) {
    ...
    // Always execute the ring, even when another renderer owns the output (the
    // native D3D9 path). The guest blocks on GPU fences / EDRAM writebacks that
    // only advance as packets are processed here; skipping execution deadlocks
    // the guest before it can even start audio. When disabled, the packets are
    // cheap -- the D3D9 hooks suppress the guest's draw and swap PM4 -- and the
    // actual present is gated in ExecutePacketType3_XE_SWAP.
...
bool CommandProcessor::ExecutePacketType3(memory::RingBuffer* reader, uint32_t packet) {
  ...
  static bool nx1_force_predicated_pm4_logged = false;
  if (!nx1_force_predicated_pm4_logged && REXCVAR_GET(nx1_force_predicated_pm4)) {
    nx1_force_predicated_pm4_logged = true;
    REXGPU_WARN("[NX1 predicate-probe] nx1_force_predicated_pm4 is enabled");
  }
  ...
  if (packet & 1) {
    ...
    bool force_predicated_pm4 =
        REXCVAR_GET(nx1_force_predicated_pm4) && opcode != PM4_XE_SWAP;
    if (!any_pass && force_predicated_pm4) {
      static uint64_t nx1_forced_predicated_packets = 0;
      ++nx1_forced_predicated_packets;
      if (nx1_forced_predicated_packets <= 64 || (nx1_forced_predicated_packets % 1000) == 0) {
        REXGPU_WARN(
            "[NX1 predicate-probe] forced predicated packet #{} opcode=0x{:02X} "
            "count={} bin_select=0x{:016X} bin_mask=0x{:016X}",
            nx1_forced_predicated_packets, opcode, count, bin_select_, bin_mask_);
      }
    } else if (!any_pass || opcode == PM4_XE_SWAP) {
      static uint64_t nx1_skipped_predicated_packets = 0;
      ++nx1_skipped_predicated_packets;
      if (nx1_skipped_predicated_packets <= 64 || (nx1_skipped_predicated_packets % 1000) == 0) {
        REXGPU_WARN(
            "[NX1 predicate-probe] skipped predicated packet #{} opcode=0x{:02X} "
            "count={} bin_select=0x{:016X} bin_mask=0x{:016X}",
            nx1_skipped_predicated_packets, opcode, count, bin_select_, bin_mask_);
      }
...
bool CommandProcessor::ExecutePacketType3_XE_SWAP(memory::RingBuffer* reader, uint32_t packet,
  ...
  // When another renderer owns the output (the native D3D9 path), execute the
  // rest of the ring as normal -- fences and writebacks must keep advancing so
  // the guest doesn't deadlock -- but do not present, so Xenia never fights the
  // D3D9 device for the window.
...
bool CommandProcessor::ExecutePacketType3_IM_LOAD(memory::RingBuffer* reader, uint32_t packet,
  ...
  // DIAG(nx1_d3d9): dump every IM_LOADed shader's ucode straight from the load
  // path. The D3D12 pipeline cache only calls DumpUcode via AnalyzeUcode, which
  // is gated per-draw (a pixel shader is skipped when its paired VS translation
  // is invalid, and skipped entirely on the async path), so some shaders the
  // guest actually binds (e.g. the menu PS 0x39EFE3599B26FBFC) never dump. Doing
  // it here guarantees complete corpus coverage for the SM3 cache regen. TODO: drop.
  if (shader && !REXCVAR_GET(dump_shaders).empty()) {
    shader->DumpUcode(REXCVAR_GET(dump_shaders));
  }
  switch (shader_type) {
