...
X_STATUS GraphicsSystem::SetupGuestGpu(runtime::FunctionDispatcher* function_dispatcher,
  ...
  // When the native D3D9 renderer (nx1_d3d9) owns the output, stop Xenia from
  // PRESENTING the guest frame -- it keeps executing the ring in full (fences,
  // EDRAM writebacks, the swap INTERRUPT packet all still fire, so the guest never
  // deadlocks), only IssueSwap is skipped. The guest frame then appears solely in
  // the D3D9 window; the host's own window still shows its imgui overlay.
  if (REXCVAR_QUERY(bool, nx1_d3d9)) {
    command_processor_->SetGpuDisabled(true);
    REXGPU_INFO("nx1_d3d9 enabled: Xenia present suppressed (ring still executes)");
  }
  ...
  vsync_worker_thread_ = system::object_ref<system::XHostThread>(
      new system::XHostThread(kernel_state_, 128 * 1024, 0, [this]() {
        ...
        const uint64_t no_vsync_interval_ticks = std::max(uint64_t(1), guest_tick_frequency / 1000);
        uint64_t last_frame_time = chrono::Clock::QueryGuestTickCount();
        while (vsync_worker_running_) {
          const bool guest_vsync_enabled = REXCVAR_GET(vsync);
          ...
          const uint64_t elapsed_ticks = current_time - last_frame_time;
          if (elapsed_ticks >= interval_ticks) {
            if (guest_vsync_enabled && elapsed_ticks >= interval_ticks * 2) {
          ...
          if (!guest_vsync_enabled) {
            rex::thread::Sleep(std::chrono::milliseconds(1));
            continue;
          }

          const uint64_t remaining_ticks = interval_ticks - elapsed_ticks;
          const uint64_t remaining_us =
              (remaining_ticks * uint64_t(1000000)) / guest_tick_frequency;
  ...
  if (auto* thread = vsync_worker_thread_->thread()) {
    thread->set_priority(rex::thread::ThreadPriority::kHighest);
  }
