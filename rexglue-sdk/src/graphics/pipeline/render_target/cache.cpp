...
REXCVAR_DEFINE_BOOL(nx1_force_scene_composite_ownership_height, false, "GPU",
                    "NX1 diagnostic: force the 1024x600 scene composite target ownership range "
                    "to cover the visible target height")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_BOOL(nx1_clamp_scene_depth_ownership_height, true, "GPU",
                    "NX1 workaround: clamp the 1024x600 scene depth target ownership range to "
                    "the visible target height")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);
      ...
      std::min(GetRenderTargetHeight(pitch_tiles_at_32bpp, msaa_samples),
               draw_extent_estimator_.EstimateMaxY(
                   interlock_barrier_only
                       ...
                       : true,
                   vertex_shader));
  const uint32_t render_target_height = GetRenderTargetHeight(pitch_tiles_at_32bpp, msaa_samples);
  if (is_rasterization_done && pitch_pixels == 1040) {
    ...
    const uint32_t nx1_scene_visible_height = std::min<uint32_t>(600, render_target_height);
    const bool nx1_scene_depth_target =
        depth_info.depth_base == 1073 && depth_info.depth_base_bit_11 == 0 &&
        (depth_and_color_rts_used_bits & 1);
    if (REXCVAR_GET(nx1_clamp_scene_depth_ownership_height) && nx1_scene_depth_target &&
        height_used > nx1_scene_visible_height) {
      height_used = nx1_scene_visible_height;
    }
    const bool nx1_scene_composite_target =
        color_info0.color_base == 98 && color_info0.color_base_bit_11 == 0 &&
        (normalized_color_mask & 0x7) == 0x7;
    if (nx1_scene_composite_target) {
      static uint64_t nx1_scene_ownership_probe_count = 0;
      ++nx1_scene_ownership_probe_count;
      const uint32_t forced_height = nx1_scene_visible_height;
      if (REXCVAR_GET(nx1_force_scene_composite_ownership_height) &&
          height_used < forced_height) {
        height_used = forced_height;
      }
      if (REXCVAR_GET(nx1_force_scene_composite_ownership_height) &&
          (nx1_scene_ownership_probe_count <= 32 ||
           (nx1_scene_ownership_probe_count % 240) == 0)) {
        const uint32_t length_used_tiles_at_32bpp_probe =
        ...
        REXGPU_WARN(
            "[NX1 rt-ownership] #{} pitch_pixels={} pitch_tiles={} msaa={} height_used={} "
            "render_height={} forced_height={} color0=0x{:08X} color_base={} depth=0x{:08X} "
            "depth_base={} z={} zw={} mask=0x{:X} length_tiles={}",
            nx1_scene_ownership_probe_count, pitch_pixels, pitch_tiles_at_32bpp,
            uint32_t(msaa_samples), height_used, render_target_height, forced_height,
            color_info0.value, uint32_t(color_info0.color_base), depth_info.value,
            uint32_t(depth_info.depth_base),
            uint32_t(normalized_depth_control.z_enable),
            uint32_t(normalized_depth_control.z_write_enable), normalized_color_mask,
            length_used_tiles_at_32bpp_probe);
      }
    }
  }
