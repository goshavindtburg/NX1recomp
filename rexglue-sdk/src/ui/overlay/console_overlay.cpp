...
void ConsoleDialog::ExecuteCommand(std::string_view cmd) {
  ...
  if (sep == std::string_view::npos) {
    const auto* info = rex::cvar::GetFlagInfo(cmd);
    if (info && info->type == rex::cvar::FlagType::Command && info->command_callback) {
      info->command_callback();
      local_entries_.push_back(
          {spdlog::level::info, "console", "[console] executed " + std::string(cmd)});
