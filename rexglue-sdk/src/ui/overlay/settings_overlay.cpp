...
void SettingsDialog::NotifyValueChanged(std::string_view name, std::string_view value) {
  if (value_changed_callback_) {
    value_changed_callback_(name, value);
  }
}
...
void SettingsDialog::OnDraw(ImGuiIO& /*io*/) {
  ...
  for (auto& entry : registry) {
    ...
    auto set_entry_value = [&](std::string value) {
      if (rex::cvar::SetFlagByName(entry.name, value)) {
        NotifyValueChanged(entry.name, value);
      }
    };
    ...
    if (is_keybind_category(entry.category)) {
      ...
      if (is_capturing) {
        ...
        if (ImGui::SmallButton("Reset##v")) {
          set_entry_value(entry.default_value);
        }
      }
      ...
      if (entry.type == rex::cvar::FlagType::Boolean) {
        ...
        if (ImGui::InputInt("##v", &v)) {
          ...
          set_entry_value(std::to_string(v));
        }
