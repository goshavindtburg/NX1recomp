...
REXCVAR_DEFINE_BOOL(d3d12_present_wait_for_vblank, false, "UI/D3D12",
                    "Wait for host vblank during D3D12 presentation for smoother 60 FPS pacing");

REXCVAR_DEFINE_BOOL(nx1_native_present_fast_paint, true, "UI/D3D12",
                    "NX1 native present: force the final guest-output paint to a single "
                    "bilinear pass.")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_BOOL(nx1_native_present_skip_duplicate_frames, true, "UI/D3D12",
                    "NX1 native present: skip non-UI D3D12 presents when the guest output has "
                    "not changed since the last successful paint.")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_BOOL(nx1_present_hitch_profiling, false, "UI/D3D12",
                    "NX1 diagnostic: log D3D12 presenter stages that exceed the hitch threshold")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_BOOL(nx1_present_hitch_log_individual, false, "UI/D3D12",
                    "NX1 diagnostic: log every presenter hitch immediately instead of "
                    "summary-only")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_UINT32(nx1_present_hitch_threshold_us, 4000, "UI/D3D12",
                      "NX1 D3D12 presenter hitch logging threshold in microseconds")
    .range(100, 1000000)
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_UINT32(nx1_present_hitch_summary_frames, 600, "UI/D3D12",
                      "NX1 D3D12 presenter hitch summary interval in presents, or 0 to disable")
    .range(0, 60000)
    .lifecycle(rex::cvar::Lifecycle::kHotReload);
...
using Nx1PresentHitchClock = std::chrono::steady_clock;

uint64_t Nx1PresentElapsedUs(Nx1PresentHitchClock::time_point start) {
  return uint64_t(std::chrono::duration_cast<std::chrono::microseconds>(
                      Nx1PresentHitchClock::now() - start)
                      .count());
}

struct Nx1PresentHitchSummaryEntry {
...
void Nx1ResetPresentHitchSummary(Nx1PresentHitchSummaryEntry* entries, size_t entry_count) {
  for (size_t i = 0; i < entry_count; ++i) {
    entries[i] = {};
  }
}

void Nx1LogPresentHitchSummary(Nx1PresentHitchSummaryEntry* entries, size_t entry_count,
                               uint64_t first_present, uint64_t last_present) {
  Nx1PresentHitchSummaryEntry* ordered[32] = {};
  ...
  std::sort(ordered, ordered + ordered_count,
            [](const Nx1PresentHitchSummaryEntry* a,
               const Nx1PresentHitchSummaryEntry* b) { return a->max_us > b->max_us; });
  REXLOG_WARN("[NX1 present hitch summary] presents={}..{} hitches={}", first_present,
              last_present, total_count);
  const uint32_t top_count = std::min<uint32_t>(ordered_count, 6);
  for (uint32_t i = 0; i < top_count; ++i) {
    const Nx1PresentHitchSummaryEntry& entry = *ordered[i];
    REXLOG_WARN(
        "[NX1 present hitch summary] #{} stage={} count={} avg={:.3f}ms max={:.3f}ms "
        "max_present={}",
        i + 1, entry.stage, entry.count, double(entry.total_us) / double(entry.count) / 1000.0,
        double(entry.max_us) / 1000.0, entry.max_present);
  }
}

void Nx1RecordPresentHitchSummary(const char* stage, uint64_t elapsed_us, uint64_t present) {
  const uint32_t summary_frames = REXCVAR_GET(nx1_present_hitch_summary_frames);
  if (!summary_frames) {
    return;
  }

  static Nx1PresentHitchSummaryEntry summaries[32];
  static uint64_t first_summary_present = 0;
  static uint64_t next_summary_present = 0;
  if (!first_summary_present) {
    first_summary_present = present;
    next_summary_present = present + summary_frames;
  }

  Nx1PresentHitchSummaryEntry* entry = nullptr;
  for (Nx1PresentHitchSummaryEntry& candidate : summaries) {
  ...
  if (entry) {
    ...
    if (elapsed_us > entry->max_us) {
      ...
      entry->max_present = present;
    }
  }

  if (present >= next_summary_present) {
    Nx1LogPresentHitchSummary(summaries, rex::countof(summaries), first_summary_present, present);
    Nx1ResetPresentHitchSummary(summaries, rex::countof(summaries));
    first_summary_present = present;
    next_summary_present = present + summary_frames;
  }
}

void Nx1LogPresentHitch(const char* stage, uint64_t elapsed_us, uint64_t present) {
  if (!REXCVAR_GET(nx1_present_hitch_profiling)) {
    return;
  }
  const uint32_t threshold_us =
      std::max<uint32_t>(1, REXCVAR_GET(nx1_present_hitch_threshold_us));
  if (elapsed_us < threshold_us) {
    return;
  }
  Nx1RecordPresentHitchSummary(stage, elapsed_us, present);
  if (REXCVAR_GET(nx1_present_hitch_log_individual)) {
    REXLOG_WARN(
        "[NX1 present hitch] stage={} elapsed={:.3f}ms present={} threshold={:.3f}ms",
        stage, double(elapsed_us) / 1000.0, present, double(threshold_us) / 1000.0);
  }
}

class Nx1ScopedPresentHitchTimer {
 public:
  Nx1ScopedPresentHitchTimer(const char* stage, uint64_t present)
      : enabled_(REXCVAR_GET(nx1_present_hitch_profiling)),
        stage_(stage),
        present_(present),
        start_(enabled_ ? Nx1PresentHitchClock::now()
                        : Nx1PresentHitchClock::time_point{}) {}

  ~Nx1ScopedPresentHitchTimer() {
    if (enabled_) {
      Nx1LogPresentHitch(stage_, Nx1PresentElapsedUs(start_), present_);
    }
  }
 ...
 private:
  ...
  const char* stage_;
  uint64_t present_;
  Nx1PresentHitchClock::time_point start_;
};
...
D3D12Presenter::ConnectOrReconnectPaintingToSurfaceFromUIThread(Surface& new_surface,
  ...
  last_painted_guest_output_refresh_sequence_ = 0;
  is_vsync_implicit_out = false;
...
Presenter::PaintResult D3D12Presenter::PaintAndPresentImpl(bool execute_ui_drawers) {
  static uint64_t nx1_present_id_counter = 0;
  const uint64_t nx1_present_id = ++nx1_present_id_counter;
  const auto nx1_present_start = Nx1PresentHitchClock::now();
  const uint64_t guest_output_refresh_sequence = GetGuestOutputRefreshSequence();

  if (!execute_ui_drawers && REXCVAR_GET(nx1_native_present_skip_duplicate_frames) &&
      guest_output_refresh_sequence != 0 &&
      guest_output_refresh_sequence == last_painted_guest_output_refresh_sequence_) {
  ...
  {
    Nx1ScopedPresentHitchTimer nx1_begin_timer("D3D12Presenter.BeginCommandList",
                                               nx1_present_id);
  ...
  Microsoft::WRL::ComPtr<ID3D12Resource> guest_output_resource;
  {
    Nx1ScopedPresentHitchTimer nx1_consume_timer("D3D12Presenter.ConsumeGuestOutput",
                                                 nx1_present_id);
    uint32_t guest_output_mailbox_index;
  ...
  if (guest_output_resource) {
    if (REXCVAR_GET(nx1_native_present_fast_paint)) {
      guest_output_paint_config.SetEffect(GuestOutputPaintConfig::Effect::kBilinear);
      guest_output_paint_config.SetDither(false);
    }
    ...
    if (guest_output_flow.effect_count) {
      const auto nx1_guest_setup_start = Nx1PresentHitchClock::now();
      ...
      Nx1LogPresentHitch("D3D12Presenter.GuestOutputSetup",
                         Nx1PresentElapsedUs(nx1_guest_setup_start), nx1_present_id);
      {
        Nx1ScopedPresentHitchTimer nx1_guest_paint_timer("D3D12Presenter.GuestOutputPaint",
                                                         nx1_present_id);
  ...
  if (execute_ui_drawers) {
    Nx1ScopedPresentHitchTimer nx1_ui_timer("D3D12Presenter.UIDraw", nx1_present_id);
    // Draw the UI.
    if (!back_buffer_bound) {
      command_list->OMSetRenderTargets(1, &back_buffer_rtv, TRUE, nullptr);
      back_buffer_bound = true;
  ...
  // Execute and present.
  {
    Nx1ScopedPresentHitchTimer nx1_execute_timer("D3D12Presenter.ExecuteCommandLists",
                                                 nx1_present_id);
    command_list->Close();
  ...
  // The default path presents immediately for low latency. NX1 benefits from
  // an optional paced path because the title expects a very steady 60 Hz cadence.
  const bool wait_for_vblank = REXCVAR_GET(vsync) && REXCVAR_GET(d3d12_present_wait_for_vblank);
  const UINT sync_interval = wait_for_vblank ? 1 : 0;
  UINT present_flags = wait_for_vblank ? 0 : DXGI_PRESENT_RESTART;
  if (!wait_for_vblank && paint_context_.swap_chain_allows_tearing) {
    present_flags |= DXGI_PRESENT_ALLOW_TEARING;
  }
  HRESULT present_result;
  {
    Nx1ScopedPresentHitchTimer nx1_present_timer("D3D12Presenter.Present", nx1_present_id);
  ...
  Nx1LogPresentHitch("D3D12Presenter.PaintAndPresent", Nx1PresentElapsedUs(nx1_present_start),
                     nx1_present_id);
  PaintResult result = PaintResult::kNotPresented;
  switch (present_result) {
    case DXGI_ERROR_DEVICE_REMOVED:
      result = PaintResult::kGpuLostExternally;
      break;
    case DXGI_ERROR_DEVICE_RESET:
      result = PaintResult::kGpuLostResponsible;
      break;
    default:
      result = SUCCEEDED(present_result) ? PaintResult::kPresented : PaintResult::kNotPresented;
      break;
  }
  if (result == PaintResult::kPresented || result == PaintResult::kPresentedSuboptimal) {
    last_painted_guest_output_refresh_sequence_ = guest_output_refresh_sequence;
  }
  return result;
}
