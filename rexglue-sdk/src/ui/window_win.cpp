...
bool Win32Window::OpenImpl() {
  ...
  raw_mouse.dwFlags = 0;
  raw_mouse.hwndTarget = hwnd_;
  if (!RegisterRawInputDevices(&raw_mouse, 1, sizeof(raw_mouse))) {
    REXLOG_WARN("RegisterRawInputDevices for mouse failed: {}", GetLastError());
  }
...
bool Win32Window::HandleRawInput(LPARAM lParam, WindowDestructionReceiver& destruction_receiver) {
  UINT size = 0;
  ...
  const int32_t delta_x = static_cast<int32_t>(mouse.lLastX);
  const int32_t delta_y = static_cast<int32_t>(mouse.lLastY);
  if (delta_x == 0 && delta_y == 0) {
    return false;
  }

  RawMouseEvent e(this, delta_x, delta_y);
...
LRESULT Win32Window::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  switch (message) {
    ...
    case WM_INPUT: {
      WindowDestructionReceiver destruction_receiver(this);
      return HandleRawInput(lParam, destruction_receiver) ? 0
                                                          : DefWindowProcW(hWnd, message, wParam,
                                                                           lParam);
    }
