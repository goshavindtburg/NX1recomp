...
#define g_AlphaTestFunction        vk::RawBufferLoad<uint>(g_PushConstants.SharedConstants + 344)
#define g_VertexIndexEndian        vk::RawBufferLoad<uint>(g_PushConstants.SharedConstants + 348)
#define g_NdcScale                 vk::RawBufferLoad<float3>(g_PushConstants.SharedConstants + 864)
#define g_NdcOffset                vk::RawBufferLoad<float3>(g_PushConstants.SharedConstants + 880)
...
#define DEFINE_SHARED_CONSTANTS() \
    ...
    uint g_Nx1SpecConstants : packoffset(c21.y); \
    uint g_AlphaTestFunction : packoffset(c21.z); \
    uint g_VertexIndexEndian : packoffset(c21.w); \
    float3 g_NdcScale : packoffset(c54); \
    float3 g_NdcOffset : packoffset(c55);

uint g_SpecConstants();

#endif

Texture1D<float4> g_Texture1DDescriptorHeap[] : register(t0, space3);
...
void Nx1AlphaTestClip(float alpha, float threshold, uint compareFunction)
{
    bool pass = false;
    if (compareFunction == 1)
        pass = alpha < threshold;
    else if (compareFunction == 2)
        pass = alpha == threshold;
    else if (compareFunction == 3)
        pass = alpha <= threshold;
    else if (compareFunction == 4)
        pass = alpha > threshold;
    else if (compareFunction == 5)
        pass = alpha != threshold;
    else if (compareFunction == 6)
        pass = alpha >= threshold;
    else if (compareFunction == 7)
        pass = true;
    clip(pass ? 1.0 : -1.0);
}
